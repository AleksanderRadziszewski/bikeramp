  ...
  async createPost(tripPost: TripPost): Promise<TripPost & TripEntity> {
    const distanceDurationData = await lastValueFrom(
      this.httpService
        .get(
          encodeURI(
            `https://api.distancematrix.ai/maps/api/distancematrix/json?origins=${tripPost.start_address}&destinations=${tripPost.destination_address}&mode=bicycling&departure_time=now&key=${process.env.DISTANCE_ACCESS_KEY}`,
          ),
        )
        .pipe(
